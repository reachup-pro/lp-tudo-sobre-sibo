// dashboard.js — orchestrator do dashboard real-time Karina / Reset Intestinal (tráfego direto contínuo)
import { sb, rpc, from, SUPABASE_URL } from '/dashboard-reset-intestinal/lib/supabase.js';
import { brl, num, pct, ago, dateBRT, timeBRT, freshness, countdown, delta, roasClass, cpaClass, applyThresholdAttr, applyThresholdCard } from '/dashboard-reset-intestinal/lib/format.js';
import { makeTimelineChart, COLORS } from '/dashboard-reset-intestinal/lib/charts.js';
import { subscribeVendas, startPolling } from '/dashboard-reset-intestinal/lib/realtime.js';

const EVENTO_SLUG = 'reset-intestinal';
...
function redirectGate() {
  location.replace('/dashboard-reset-intestinal/acesso-restrito.html');
}
...
function renderHeader() {
  setText('[data-evt-nome]', state.kpis?.produto_nome || 'Reset Intestinal');
...
function renderHero() {
  const k = state.kpis;
  if (!k) return;
  // Produto perene (tráfego direto): hero mostra vendas totais + receita, sem lotes/countdown
  setText('[data-hero-vendas]', num(k.vendas_count || 0));
...
function renderTimeline(rows) {
  ...
  // Produto perene: sem meta dinâmica de evento — timeline só mostra vendas × investimento
  const meta = null;
...
function renderFeed() {
  ...
  container.innerHTML = state.feed.slice(0, 25).map(v => `
    <div class="feed__item">
      ...
      <span class="feed__produto" title="${escapeHtml(v.produto || '')}">${escapeHtml(v.produto_label || (v.produto === 'Reset Intestinal' ? 'Produto' : '—'))}</span>
